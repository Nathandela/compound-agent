...
- Reasoning: Named function substitution. Can detect `isModelUsable(` via regex in `go/internal/**/*.go`.
...
Before creating Class B tasks, detect the project's linter by checking for config files (first match wins):

1. `.golangci.yml` / `.golangci.yaml` -> golangci-lint
2. `eslint.config.*` / `.eslintrc.*` -> eslint
3. `ruff.toml` / `.ruff.toml` / `pyproject.toml` with `[tool.ruff]` -> ruff
4. `clippy.toml` / `.clippy.toml` -> clippy

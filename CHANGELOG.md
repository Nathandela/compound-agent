...
- **Empty hook arrays in `settings.json`**: `AddHooksForProfile` now drops empty `"PreToolUse": []` style entries at the end of the install (previously created eagerly by `upgradeNpxHooks` calling `getHookArray` for every known hook type). Cosmetic only; no behavioral change for in-profile hooks.

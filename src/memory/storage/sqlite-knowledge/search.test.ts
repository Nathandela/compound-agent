...
import { searchChunksKeyword, searchChunksKeywordScored } from './search.js';
...
describe('searchChunksKeyword', () => {
  let repo: string;

  beforeEach(async () => {
    repo = await mkdtemp(join(tmpdir(), 'knowledge-search-'));
  });
  ...
  it('returns matching chunks', () => {
    ...
    const results = searchChunksKeyword(repo, 'authentication', 10);
    expect(results).toHaveLength(1);
    expect(results[0].id).toBe('C001');
  });

  it('returns empty array for empty database', () => {
    // Open DB but don't insert anything
    openKnowledgeDb(repo, { inMemory: true });
    const results = searchChunksKeyword(repo, 'anything', 10);
    expect(results).toEqual([]);
  });

  it('returns empty for sanitized-away query', () => {
    insertChunk(repo, 'C001', 'docs/api.md', 'some content');
    const results = searchChunksKeyword(repo, '"*^', 10);
    expect(results).toEqual([]);
  });
  ...
  it('respects the limit parameter', () => {
    ...
    const results = searchChunksKeyword(repo, 'typescript', 2);
    expect(results).toHaveLength(2);
  });

  it('does not crash on special FTS5 characters', () => {
    insertChunk(repo, 'C001', 'docs/a.md', 'test content');
    const results = searchChunksKeyword(repo, '"hello* ^world"', 10);

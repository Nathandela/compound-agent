...
export async function runUpdate(repoRoot: string, dryRun: boolean): Promise<{
  ...
  // Dynamic stale artifact cleanup (replaces hardcoded DEPRECATED_PATHS/HOW_TO_COMPOUND)
  const staleRemoved = await cleanStaleArtifacts(repoRoot, dryRun);
  const staleGeminiRemoved = await cleanStaleGeminiArtifacts(repoRoot, dryRun);
  ...
  setupCommand
    ...
    .action(async function (this: Command, options: {
      ...
      if (options.update) {
        ...
        if (result.staleRemoved.length > 0) {
          console.log(`  ${prefix}Removed stale: ${result.staleRemoved.length} artifact(s)`);
          for (const path of result.staleRemoved) {
            console.log(`    - ${path}`);
          }
        }
